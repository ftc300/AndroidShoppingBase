package com.shoping.mall.dao.base;
...
public abstract class DAOSupport<M> implements DAO<M> {
	...
	public DAOSupport(Context context) {
		...
		helper = new DBHelper(GlobalParams.CONTEXT);
	...
	public int delete(Serializable id) {
		return db.delete(getTableName(), DBHelper.TABLE_HELP_ID + " =?", new String[] { id.toString() });
	}
	...
	public int update(M m) {
		ContentValues values = new ContentValues();
		fillColumn(m, values);
		return db.update(getTableName(), values, DBHelper.TABLE_HELP_ID + " =?", new String[] { getId(m) });
	}
