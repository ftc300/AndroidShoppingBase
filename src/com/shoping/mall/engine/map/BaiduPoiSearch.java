...
public class BaiduPoiSearch {
		...
		OnGetPoiSearchResultListener poiListener = new OnGetPoiSearchResultListener(){
		    ...
		    public void onGetPoiDetailResult(PoiDetailResult result){  
				...
				if (result.error != SearchResult.ERRORNO.NO_ERROR) {
					...
					LogUtil.i( TAG + " 详情检索失败  ");
				} else {
					// 检索成功
					LogUtil.i( TAG + " 详情检索成功 ");
					String address = result.getAddress();
					String name = result.getName();
					String phone = result.getTelephone();
					LogUtil.i( TAG + " 名称：" + name + " 电话：" + phone + " 地址：" + address);
				}
	...
	public void startSearchNearBy(double lat,double log,String keyword) {
			if(!TextUtils.isEmpty(keyword)){
				...
				LogUtil.i( TAG + " 开始在 经度：" + log + "纬度：" + lat + "关键字：" + keyword);
			}
			else{
				LogUtil.i( TAG + " 搜索的关键字为空");
			}
	}
